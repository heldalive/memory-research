...
Loop: research-1789656281930. Stage: deliverable_review.
...
Latest instance: research-1789656281930-09-researcher (completed).

Execute a bounded research task: Identify and inspect sources reporting descriptive performance metrics (success rate, token usage, hallucination rate) on a single, reproducible benchmark (e.g., BigBench) under fixed token budgets. Explicitly document where direct baseline comparisons are missing. Limit to 4 sources; do not implement code.

See [status](STATUS.md) and the versioned [instance](../../instances/research-1789656281930-09-researcher.json). At most two plan reviews and two deliverable reviews; no implementation is enabled.
